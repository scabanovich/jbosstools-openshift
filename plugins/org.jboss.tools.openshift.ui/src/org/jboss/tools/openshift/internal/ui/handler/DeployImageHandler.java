...
public class DeployImageHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if(image == null) {
			return OpenShiftUIActivator.statusFactory().cancelStatus("No Docker Image selected that we can deploy."); //$NON-NLS-1$
		}
		WizardUtils.openWizardDialog(new DeployImageWizard(), HandlerUtil.getActiveShell(event));
